--マドルチェ・プロムナード
function c101012076.initial_effect(c)
	--activate
	...
	e1:SetCountLimit(1,101012076)
	e1:SetTarget(c101012076.target)
	e1:SetOperation(c101012076.activate)
	c:RegisterEffect(e1)
	--attach
	...
	e2:SetCountLimit(1,101012076)
	e2:SetCost(aux.bfgcost)
	e2:SetTarget(c101012076.mattg)
	e2:SetOperation(c101012076.matop)
	c:RegisterEffect(e2)
end
c101012076.listed_series={0x71}
function c101012076.thfilter(c)
...
function c101012076.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g2=Duel.SelectTarget(tp,c101012076.thfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g2,1,0,0)
end
function c101012076.activate(e,tp,eg,ep,ev,re,r,rp)
...
function c101012076.xyzfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x71) and c:IsType(TYPE_XYZ)
end
function c101012076.matfilter(c)
	return c:IsSetCard(0x71) and c:IsType(TYPE_MONSTER)
end
function c101012076.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c101012076.xyzfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101012076.xyzfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(c101012076.matfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c101012076.xyzfilter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c101012076.matop(e,tp,eg,ep,ev,re,r,rp)
