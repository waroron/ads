...
function c101012012.initial_effect(c)
	...
	e2:SetCountLimit(1,101012012+100)
	e2:SetTarget(c101012012.thtg)
	e2:SetOperation(c101012012.thop)
	c:RegisterEffect(e2)
end
function c101012012.tdfilter(c)
	return c:IsFaceup() and c:IsAbleToDeck() and not c:IsCode(101012012)
end
function c101012012.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c101012012.tdfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101012012.tdfilter,tp,LOCATION_REMOVED,0,1,nil)
...
function c101012012.thfilter(c)
	return c:IsSetCard(0x23e) and c:IsType(TYPE_MONSTER) and not c:IsCode(101012012) and c:IsAbleToHand()
end
function c101012012.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REMOVED) and c101012012.thfilter(chkc) end
