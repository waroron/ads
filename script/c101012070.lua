...
function c101012070.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b2=c101012070.sptg(e,tp,eg,ep,ev,re,r,rp,0)
	if chk==0 then return b1 or b2 end
	local stable={}
	local dtable={}
	if b1 then
		table.insert(stable,1)
		table.insert(dtable,aux.Stringid(101012070,0))
	end
	if b2 then
		table.insert(stable,2)
		table.insert(dtable,aux.Stringid(101012070,1))
	end
	...
	if op==1 then
		...
		e:SetOperation(c101012070.atkop)
	elseif op==2 then
		e:SetCategory(CATEGORY_SPECIAL_SUMMON)
		e:SetOperation(c101012070.spop)
		c101012070.sptg(e,tp,eg,ep,ev,re,r,rp,1)
	end
end
function c101012070.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	g:ForEach(c101012070.op,e:GetHandler(),atk)
end
function c101012070.op(tc,c,atk)
	local e1=Effect.CreateEffect(c)
...
function c101012070.filter(c,e,tp,m,m2,ft)
	...
	if ft>0 then
		...
		res=mg:IsExists(c101012070.filterf,1,nil,tp,mg,c)
	end
	return res
end
function c101012070.filterf(c,tp,mg,rc)
	if c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:GetSequence()<5 then
		Duel.SetSelectedCard(c)
		return mg:CheckWithSumEqual(Card.GetOriginalLevel,rc:GetOriginalLevel()*2,0,99,rc)
	else return false end
end
function c101012070.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		local mg=Duel.GetRitualMaterial(tp)
		local mg2=Group.CreateGroup()
		local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
		return ft>-1 and Duel.IsExistingMatchingCard(c101012070.filter,tp,LOCATION_DECK,0,1,nil,e,tp,mg,mg2,ft)
	end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c101012070.spop(e,tp,eg,ep,ev,re,r,rp)
	local mg=Duel.GetRitualMaterial(tp)
	local mg2=Group.CreateGroup()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tg=Duel.SelectMatchingCard(tp,c101012070.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp,mg,mg2,ft)
	...
	if tc then
		...
		if tc.ritual_custom_operation then
			...
			if ft>0 then
				...
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
				mat=mg:FilterSelect(tp,c101012070.filterf,1,1,nil,tp,mg,tc)
