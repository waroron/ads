...
function c101012058.initial_effect(c)
	...
	e1:SetCountLimit(1,511027049)
	e1:SetTarget(c101012058.target)
	e1:SetOperation(c101012058.activate)
	c:RegisterEffect(e1)
end
function c101012058.tpfilter(c)
	return c:IsType(TYPE_LINK)
end
function c101012058.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c101012058.tpfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingTarget(aux.TRUE,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELF)
	local g1=Duel.SelectTarget(tp,c101012058.tpfilter,tp,LOCATION_MZONE,0,1,1,nil)
...
function c101012058.activate(e,tp,eg,ep,ev,re,r,rp,g)
